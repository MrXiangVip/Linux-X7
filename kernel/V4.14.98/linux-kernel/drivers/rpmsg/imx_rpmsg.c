...
/* sram0 base address for storing virtqueue structure */
...
#else
#define imx_rpmsg_noirq_suspend NULL
#define imx_rpmsg_noirq_resume NULL
#endif

#ifdef CONFIG_PM
static int pm_imx_rpmsg_freeze(struct device *dev)
{
	struct imx_rpmsg_vproc *rpdev = &imx_rpmsg_vprocs[0];
	struct virtqueue *_vq;
	struct vring_virtqueue *vq;
	size_t size = sizeof(struct vring_virtqueue);
	int i, vdevid;

	if (!sram_base)
		sram_base = ioremap(0x2F000000, SZ_4K);

	for (vdevid = 0; vdevid < rpdev->vdev_nums; ++vdevid)
		for (i = 0; i < 2; ++i) {
			_vq = rpdev->ivdev[vdevid]->vq[i];
			vq = to_vvq(_vq);
			memcpy((void*)((u32)sram_base
				+ (i + vdevid*2)*size), (void*)vq, size);
		}

	/* must unmap the SRAM to avoid page swap */
	iounmap(sram_base);
	sram_base = NULL;
...
static int pm_imx_rpmsg_restore(struct device *dev)
{
	struct imx_rpmsg_vproc *rpdev = &imx_rpmsg_vprocs[0];
	struct virtqueue *_vq;
	struct vring_virtqueue *vq;
	...
	/* remap the SRAM region to restore the vq */
	if (!sram_base)
		sram_base = ioremap(0x2F000000, SZ_4K);
	...
	for (vdevid = 0; vdevid < rpdev->vdev_nums; ++vdevid)
		for (i = 0; i < 2; ++i) {
			_vq = rpdev->ivdev[vdevid]->vq[i];
			vq = to_vvq(_vq);
			memcpy((void*)vq, (void*)((u32)sram_base
				+ (i + vdevid*2)*size), size);
		}
...
#else
#define pm_imx_rpmsg_restore NULL
#define pm_imx_rpmsg_freeze NULL
#endif
...
#ifdef CONFIG_PM
	.freeze_late = pm_imx_rpmsg_freeze,
	.restore_early = pm_imx_rpmsg_restore,
#endif
};
