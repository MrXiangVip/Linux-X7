...
#define DETECTOR_DEVICE			"detector"
#define SENSORHUB_DEVICE			"sensorhub"

#define SENSORHUB_TYPE			0x0
#define SENSORHUB_IDX			0x0
...
struct rpmsg_input_data {
	...
	struct miscdevice *sensorhub_miscdev;
	struct input_dev *detector_input;
	struct input_dev *sensorhub_input;
	atomic_t detector_active;
	atomic_t sensorhub_active;
	atomic_t sensorhub_delay;
...
static inline void rpmsg_input_evt_report(struct rpmsg_input_msg *msg)
{
	...
	} else if (msg->header.cmd ==  RPMSG_INPUT_SENSORHUB_CMD) {
		val = msg->instruct.val[0];
		input_report_abs(input_rpmsg->sensorhub_input, ABS_X, val);
		val = msg->instruct.val[1];
		input_report_abs(input_rpmsg->sensorhub_input, ABS_Y, val);
		val = msg->instruct.val[2];
		input_report_abs(input_rpmsg->sensorhub_input, ABS_Z, val);
		// printk("x = %d , y = %d , z = %d \n", msg->instruct.val[0], msg->instruct.val[1], msg->instruct.val[2]);
		val = msg->instruct.val[3];
		input_report_abs(input_rpmsg->sensorhub_input, ABS_DISTANCE, val);
		val = msg->instruct.val[4];
		input_report_abs(input_rpmsg->sensorhub_input, ABS_MISC, val);
		input_sync(input_rpmsg->sensorhub_input);
	}
}
...
static struct miscdevice detector_device = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "detector",
	.fops = &rpmsg_input_fops,
};

static struct miscdevice sensorhub_device = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "sensorhub",
	.fops = &rpmsg_input_fops,
};
...
static int rpmsg_input_change_mode(int cmd, int enable)
{
	...
	msg.header.cmd = cmd;
	msg.sensor_type = SENSORHUB_TYPE;
	msg.sensor_index = SENSORHUB_IDX;
				       ...
				       char *buf)
{
	...
	if (pdata->sensorhub_miscdev == misc_dev)
		enable = atomic_read(&pdata->sensorhub_active);
					...
					const char *buf, size_t count)
{
	...
	if (misc_dev == pdata->sensorhub_miscdev)
		cmd = RPMSG_INPUT_SENSORHUB_CMD;
	...
	if (!ret) {
		...
		else if (cmd == RPMSG_INPUT_SENSORHUB_CMD)
			atomic_set(&pdata->sensorhub_active, enable);
	}
					   ...
					   char *buf)
{
	...
	if (pdata->sensorhub_miscdev == misc_dev)
		poll_delay = atomic_read(&pdata->sensorhub_delay);
					    ...
					    const char *buf, size_t count)
{
	...
	if (!ret) {
		if (cmd == RPMSG_INPUT_POLL_DELAY_CMD)
			atomic_set(&pdata->sensorhub_delay, delay);
	}
...
static int rpmsg_input_register_sysfs_device(struct rpmsg_input_data *pdata)
{
	...
	/* register sysfs for sensorhub */
	misc_dev = pdata->sensorhub_miscdev;
	err = sysfs_create_group(&misc_dev->this_device->kobj,
				 &rpmsg_input_attr_group);
	if (err)
...
static int rpmsg_input_device_probe(struct platform_device *pdev)
{
	...
	sensorhub_input = devm_input_allocate_device(dev);
	if (!sensorhub_input) {
		dev_err(dev, "failed to allocate sensorhub device\n");
		return -ENOMEM;
	}
	...
	sensorhub_input->name = SENSORHUB_DEVICE;
	sensorhub_input->phys = "rpmsg-input/input1";
	sensorhub_input->dev.parent = &pdev->dev;
	sensorhub_input->id.bustype = BUS_HOST;
	sensorhub_input->evbit[0] = BIT_MASK(EV_ABS);
	input_set_abs_params(sensorhub_input, ABS_X, ABSMIN_ACC_VAL, ABSMAX_ACC_VAL, 0, 0);
	input_set_abs_params(sensorhub_input, ABS_Y, ABSMIN_ACC_VAL, ABSMAX_ACC_VAL, 0, 0);
	input_set_abs_params(sensorhub_input, ABS_Z, ABSMIN_ACC_VAL, ABSMAX_ACC_VAL, 0, 0);
	input_set_abs_params(sensorhub_input, ABS_DISTANCE, 0, 0xFFFFFFFF, 0, 0);
	input_set_abs_params(sensorhub_input, ABS_MISC, 0, 0xFFFFFFFF, 0, 0);
	pdata->sensorhub_input = sensorhub_input;

	input_rpmsg = pdata;
	platform_set_drvdata(pdev, pdata);

	ret = misc_register(&detector_device);
	if (ret != 0) {
		dev_err(dev, "register acc miscdevice error");
		goto err_register_detector_misc;
	}
	pdata->detector_miscdev = &detector_device;

	ret = misc_register(&sensorhub_device);
	if (ret != 0) {
		dev_err(dev, "register acc miscdevice error");
		goto err_register_sensorhub_misc;
	}
	pdata->sensorhub_miscdev = &sensorhub_device;
	...
	ret = input_register_device(sensorhub_input);
	if (ret) {
		dev_err(dev, "Unable to register sensorhub device, error: %d\n",
			ret);
		goto err_register_sensorhub_input;
	}
...
err_register_rmpsg_driver:
	input_unregister_device(sensorhub_input);
err_register_sensorhub_input:
	input_unregister_device(detector_input);
err_register_detector_input:
err_register_sysfs:
	misc_deregister(&sensorhub_device);
	pdata->sensorhub_miscdev = NULL;
err_register_sensorhub_misc:
	misc_deregister(&detector_device);
	pdata->detector_miscdev = NULL;
err_register_detector_misc:
	pdata->detector_input = NULL;
	pdata->sensorhub_input = NULL;
