...
//<<<start by tanqw 20191015
#define CAMERA_OV7725_QVGA			1
#define CAMERA_OV7725_VGA			0
#define CAMERA_OV7956			0
#define CAMERA_ORBBEC			0
#define CAMERA_HIMAX			0
#if ((CAMERA_OV7725_QVGA + CAMERA_OV7725_VGA + CAMERA_OV7956 + CAMERA_ORBBEC + CAMERA_HIMAX) > 1)
	#error "CAMERA_OV7725_QVGA and CAMERA_OV7725_VGA and CAMERA_OV7956 and CAMERA_ORBBEC, CAMERA_HIMAX can't be defined at the same time!"
#endif
//>>>end by tanqw 20191015
...
static int __imx_viu_set_fmt(u32 viu_base, u32 mbus_code, u32 width, u32 height)
{
	uint32_t scr, invsz, vid_size, ext_config;
	void __iomem *base = (void __iomem *)viu_base;

	scr = readl(base + VIU_SCR);
	scr &= ~SCR_SET_FORMAT_CTRL(0x7);
	scr &= ~(SCR_MODE444 | SCR_YUV2RGB_EN);

	ext_config = readl(base + VIU_EXT_CONFIG);
	ext_config &= ~EXT_CONFIG_SET_INP_FORMAT(0x7);
	ext_config |= EXT_CONFIG_LENDIAN;
	ext_config |= EXT_CONFIG_HSYNC_POL_ACTIVE_LOW;
...
#endif
		invsz = INVSZ_SET_LINEC(height) |
			INVSZ_SET_PIXELC(width);
		vid_size = VID_SIZE_SET_LINEC(height) |
			   VID_SIZE_SET_PIXELC(width);
		/* ITU-656 stream */
	...
	case MEDIA_BUS_FMT_RGB565_1X16:
		...
		vid_size = VID_SIZE_SET_LINEC(height) |
			   VID_SIZE_SET_PIXELC(width);
		/* 16bit parallel stream */
		ext_config |= EXT_CONFIG_SET_INP_FORMAT(0x7);
		/* big endian */
		ext_config &= ~EXT_CONFIG_DE_VALID;
		break;
	default:
		pr_err("%s: unsupported mbus fmt: %#x\n",
			__func__, mbus_code);
		return -EINVAL;
	}

	writel(ext_config, base + VIU_EXT_CONFIG);
	writel(invsz, base + VIU_INVSZ);
	writel(vid_size, base + VIU_VID_SIZE);
	writel(scr, base + VIU_SCR);
...
static int imx_viu_set_fmt(struct imx_viu_device *viu_dev)
{
	...
	return __imx_viu_set_fmt((u32)viu_dev->base, viu_fmt->mbus_code,
...
static int imx_viu_async_subdevs_register(struct imx_viu_device *viu_dev)
{
	...
	struct device_node *np = viu_dev->dev->of_node;
...
#if CAMERA_OV7956
	__imx_viu_set_fmt((u32)viu_dev->base, MEDIA_BUS_FMT_YUYV8_2X8, 640, 480); //ov7956
#endif
#if CAMERA_OV7725_VGA
	__imx_viu_set_fmt((u32)viu_dev->base, MEDIA_BUS_FMT_RGB565_1X16, 480, 1280); //ov7725 vga
#endif
#if CAMERA_OV7725_QVGA
	__imx_viu_set_fmt((u32)viu_dev->base, MEDIA_BUS_FMT_RGB565_1X16, 240, 640); //ov7725 qvga
#endif
#if CAMERA_ORBBEC
	__imx_viu_set_fmt((u32)viu_dev->base, MEDIA_BUS_FMT_RGB565_1X16, 480, 640); //obbec
#endif
#if CAMERA_HIMAX
	__imx_viu_set_fmt((u32)viu_dev->base, MEDIA_BUS_FMT_RGB565_1X16, 1280, 800);  //himax
#endif	
