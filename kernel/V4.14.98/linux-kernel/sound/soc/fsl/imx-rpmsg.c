...
static int imx_rpmsg_probe(struct platform_device *pdev)
{
	...
	if (codec_np) {
		...
		if (!codec_dev || !codec_dev->dev.driver) {
			dev_err(&pdev->dev, "failed to find i2c codec device\n");
			ret = -EINVAL;
			goto fail;
		}
	}
	...
	if (codec_np) {
		data->dai[1].name = "rpmsg cx20921";
		data->dai[1].stream_name = "rpmsg cx20921";
		data->dai[1].codec_dai_name = "cx2092x-aif";
		data->dai[1].codec_of_node = codec_np;
		data->dai[1].cpu_dai_name = dev_name(&cpu_pdev->dev);
		data->dai[1].platform_of_node = cpu_np;
