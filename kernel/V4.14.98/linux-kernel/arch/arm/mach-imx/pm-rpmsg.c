...
static int pm_hibernate_notify_m4(u8 mode)
{
	struct pm_rpmsg_data msg;
...
static int pm_heartbeat_freeze(struct device *dev)
{
	int err;

	err = pm_hibernate_notify_m4(PM_RPMSG_FREEZE);
	if (err)
		return err;
...
static int pm_heartbeat_restore(struct device *dev)
{
	int err;

	err = pm_hibernate_notify_m4(PM_RPMSG_RESTORE);
	if (err)
		return err;
...
static int pm_heartbeat_thaw(struct device *dev)
{
	int err;

	err = pm_hibernate_notify_m4(PM_RPMSG_THAW);
	if (err)
		return err;
...
static const struct dev_pm_ops pm_heartbeat_ops = {
	.suspend_late = pm_heartbeat_suspend,
	.resume_early = pm_heartbeat_resume,
	.freeze_late = pm_heartbeat_freeze,
	.thaw_early = pm_heartbeat_thaw,
	.poweroff_late = pm_heartbeat_suspend,
	.restore = pm_heartbeat_restore,
};
