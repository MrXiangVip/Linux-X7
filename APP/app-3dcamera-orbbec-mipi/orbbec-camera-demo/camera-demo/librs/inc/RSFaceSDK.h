...
extern "C" {
  ...
  RS_SDK_API void STDCALL rsUnInitLivenessDetect(RSHandle *handle);
    ...
    RS_SDK_API int STDCALL rsRunDepthLivenessDetect(RSHandle handle, const unsigned short *image, 
          int width, int height, int stride,
          rs_face *pFace, int *livenessState);
    ...
    // MutiLiveness Detect
    //////////////////////////////
    RS_SDK_API void STDCALL rsInitMutiLivenessDetect(RSHandle *handle, RSHandle lm);

    RS_SDK_API int STDCALL rsRunMutiLivenessDetect(RSHandle handle, const unsigned char *image1, const unsigned short *image2, rs_pixel_format pix_format,  
    int width, int height, int stride, rs_face *pFace, int *livenessState);
 
    RS_SDK_API void STDCALL rsUnInitMutiLivenessDetect(RSHandle *handle);
    

    RS_SDK_API bool STDCALL rsIsMouseOpen(const unsigned char *image, rs_pixel_format pix_format, int width, int height, int stride, rs_face *pFace);

    RS_SDK_API int STDCALL  rsRunDepthFaceDistance(RSHandle handle, const unsigned short *image, int width, int height, int stride, rs_face *pFace);
