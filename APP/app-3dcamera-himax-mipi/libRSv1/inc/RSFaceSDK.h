...
extern "C" {
  ...
  typedef struct rs_face_attr
  {
	...
	// License Manager
	////////////////////////////
	RS_SDK_API int STDCALL rsInitLicenseManager(RSHandle *handle, const char *appId, const char* appSecret);
	RS_SDK_API const char *rsGetSDKLicenseDeviceKey(); // must be called after init
	RS_SDK_API void STDCALL rsUnInitLicenseManager(RSHandle *handle);
