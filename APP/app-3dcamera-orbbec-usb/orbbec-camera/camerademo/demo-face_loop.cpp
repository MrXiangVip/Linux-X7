...
#include "bmp_facewaiting.cpp"  //hannah added for demo
#include "bmp_detecready.cpp" 

//#define PER_TIME_CAL  //190323
...
static const char *rpmsgdev = "/dev/ttyRPMSG30";  
...
// 0 - register
// 1 - register done
//static void face_update_banner(int32_t banner_type)
static int32_t face_update_banner(int32_t banner_type)
{
	...
	case BANNER_LIVENESS_FAIL:
		...
		break;
	case BANNER_FACEWAITING:
		bmp_data = facewaiting_bmp;
		break;	
	case BANNER_DETECIDLE:
		bmp_data = detecready_bmp;
		break;
	default:
		break;
	}
...
//	bool livenessRun = true;  //hannah move out
	...
	while (livenessRun) {
		...
		case LIVENESS_INIT:
			{
				...
				//face_recognize_change_liveness_detect_status(LIVENESS_START);  //hannah restored
				face_recognize_change_liveness_detect_status(LIVENESS_IDLE);  //hannah modified
				cout << "LIVENESS_INIT done, set to LIVENESS_IDLE!" << endl;  //0415				
			}
			break;
		case LIVENESS_START:
			{
				...
				cout << "--->LIVENESS_START done: " << count << endl;  //hannah added
				count ++;
		...
		case LIVENESS_CAPTURE:
			{
				...
				if ((1 == faceBuf.color_filled) && (0 == faceBuf.depth_filled) && faceBuf.depth_mutex.try_lock()) {//0411
		...
		case LIVENESS_STOP:
			...
			cout << "--->LIVENESS_STOP done: " << count << endl;  //hannah added
			count ++;
			break;
		case LIVENESS_IDLE:
			break;
...
//	bool detectRun = true;  //hannah move out
	...
	while (detectRun) {
		...
		case FACE_DETECT_INIT:
			...
			cout << "FACE_DETECT_INIT done, set to FACE_DETECT_IDLE!" << endl;
			//face_recognize_change_face_detect_status(FACE_DETECT_RECOGNIZE_INIT);
			face_recognize_change_face_detect_status(FACE_DETECT_IDLE);  //hannah modified
			face_update_banner(BANNER_DETECIDLE); //added 190326, ready for face register of recognize				
			break;
		case FACE_DETECT_RECOGNIZE_INIT:
			face_recognize_change_face_detect_status(FACE_DETECT_RECOGNIZE);
			if (banner_address_1 && banner_address_2)  //added 190325
				face_update_banner(BANNER_RECOGNIZING);		
		...
		case FACE_DETECT_RECOGNIZE:
			{
				...
				face_recognize_change_liveness_detect_status(LIVENESS_START);  //hannah deleted 190415
				...
				if (RS_RET_NO_FACE == faceId) {
					...
					//face_update_banner(BANNER_LIVENESS_FAIL);
				} else if (RS_RET_NO_RECO == faceId) {
					cout << "face OK but no record in DB!" << endl;
					//face_update_banner(BANNER_RECOGNIZE_FAIL);
				} else if (faceId > 0) { //face recogized ok				
					cout << "face recogized success!" << endl;
				#ifdef LIVENESS_DETECT_SUPPORT
					face_recognize_change_liveness_detect_status(LIVENESS_STOP);  //IDLE-->STOP
				#endif					
					face_recognize_change_face_detect_status(FACE_DETECT_IDLE);  //also used by register
					face_update_banner(BANNER_RECOGNIZED);				
				...
				if(recogtime > 20)
				{
					...
					face_recognize_change_liveness_detect_status(LIVENESS_STOP);  //IDLE-->STOP
				#endif
					face_recognize_change_face_detect_status(FACE_DETECT_IDLE);			
					face_update_banner(BANNER_RECOGNIZE_FAIL);					
		...
		case FACE_DETECT_REGISTER_INIT:
			...
			if (banner_address_1 && banner_address_2)   //190325
				face_update_banner(BANNER_REGISTERING);			
		...
		case FACE_DETECT_REGISTER:
			{
				...
				if (RS_RET_OK == ret) {
					...
					face_recognize_change_liveness_detect_status(LIVENESS_STOP);  //IDLE-->STOP
				#endif						
					face_recognize_change_face_detect_status(FACE_DETECT_IDLE);	//hannah				
					face_update_banner(BANNER_REGISTER_OK);					
				...
				#endif	
					face_recognize_change_face_detect_status(FACE_DETECT_IDLE);	 //hannah				
					face_update_banner(BANNER_REGISTERED);			
					RegResp[3] = 0;  //(unsigned char)faceId;
					len = sizeof(RegResp);
					fd = open(rpmsgdev, O_RDWR);  //open RPmsg virtual tty device		
					if(fd > 0)
					{
				...
				#endif							
					face_recognize_change_face_detect_status(FACE_DETECT_IDLE);	//hannah???			
					face_update_banner(BANNER_REGISTER_FAIL);									
		...
		case FACE_DETECT_IDLE:	
			sleep(1);   //hannah 190325
			face_update_banner(BANNER_FACEWAITING); 
			break;
...
//	int32_t previewRun = true;
	...
	while (previewRun) {
		...
		case PREVIEW_INIT:
			...
			cout << "PREVIEW_INIT OK!" << endl;
			//face_update_banner(BANNER_RECOGNIZING);  //hannah deleted in190325
...
int main(int argc, char **argv)
{
...
#endif
	int32_t l_ret = -1, fd = -1;
	int32_t key_fd = 0;
	Status rc = STATUS_ERROR;
	struct input_event key_event;
	unsigned int usr_val=0;  //para to kernel
...
#ifdef LIVENESS_DETECT_SUPPORT
	face_recognize_change_liveness_detect_status(LIVENESS_INIT);  //hannah marked
#endif
	...
	while(1)
	{
		fd = open(rpmsgdev, O_RDWR);
		if(!fd)
			printf("rpmsgdev open failed!");
		l_ret = ioctl(fd, RPMSG_READ, &usr_val);  //transfer usr_val addr to kernel
		if(l_ret < 0)
		{
			printf("rpmsg ioctl error! exit loop!\n");
			break;
		}
		else
		{		
			close(fd);
			memcpy(rpmsg_buf, &usr_val, 4);
		}

		if((rpmsg_buf[0] == 0x4d) && (rpmsg_buf[1] == 0x34))
		{
			printf("rpmsg_buf: 0x %02x %02x %02x %02x\n", rpmsg_buf[0], rpmsg_buf[1], rpmsg_buf[2], rpmsg_buf[3]);
			switch(rpmsg_buf[2])
			{
				case CMD_FACE_RECOGNITION:
					cout << "Switch to face recognize..." << endl;		
#ifdef LIVENESS_DETECT_SUPPORT
					face_recognize_change_liveness_detect_status(LIVENESS_START);  //hannah added 0415
#endif						
					face_recognize_change_face_detect_status(FACE_DETECT_RECOGNIZE_INIT);					
					break;
