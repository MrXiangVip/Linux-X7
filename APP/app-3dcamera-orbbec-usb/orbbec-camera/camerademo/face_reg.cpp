...
static bool livenessRun = true;  //add liveness in register 190403
...
static int32_t face_op_status = FACE_REGISTER;  //useless
...
static void faceDetectProc(void)
{
	...
	int recogtime = 0;  
	unsigned char faceId_buf[2]={0};  //hannah added for test
...
//			cout << "FACE_DETECT_INIT done!" << endl;
			face_recognize_change_face_detect_status(FACE_DETECT_REGISTER_INIT);  //for register purpose
			break;
...
//				cout << "RECOGNIZE--faceId: " << faceId << endl;
		...
		case FACE_DETECT_REGISTER:
			...
			#if 0	//just for test									
					faceId_buf[2] = (unsigned char)faceId;
					//add face id to specific file
					FILE *fp = NULL;
					fp = fopen("faceID-record", "w+");
					fwrite(faceId_buf, 2, 1, fp);  //???
					fclose(fp);		
					{
						...
						fwrite(faceBuf.color_buf, CAM_WIDTH * CAM_HEIGHT * 4, 1, fp);
						fclose(fp);
					}
			#endif		
...
//					cout << "face already exists in database!" << endl;					face_recognize_update_banner(BANNER_REGISTERED);
