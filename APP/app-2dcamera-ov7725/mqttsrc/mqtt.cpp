...
#define REG_IMG_DIR "."
#else
#define REG_IMG_DIR "/opt/smartlocker/pic"
#endif
...
// 远程开锁
char mqtt_msg_id_unlock[MQTT_MSG_ID_LEN];
...
char *getFirmwareVersion() {
	return "FW_W7_SVN260_20210207_V1.0.0";
}
...
int sendStatusToMCU(int biz, int ret) {
	...
	log_info("sendStatusToMCU %s", payload_str);
...
int handlePayload(char *payload, char *msg_idStr) {
	if (payload != NULL) {
		...
		if (x7_cmd_code == 0x83) {
			alarm(20);
			memset(mqtt_msg_id_unlock, '\0', MQTT_MSG_ID_LEN);
			log_debug("------ handlePayload unlock msg_idStr is %s\n", msg_idStr);
			if (msg_idStr != NULL && strlen(msg_idStr) > 0) {
				strcpy(mqtt_msg_id_unlock, msg_idStr);
				log_debug("------ cp unlock msg_idStr done\n");
			}
		} else {
			alarm(5);
		}
...
int sendImage(const char *filename, char *uuidStr, int needSendMCU) {
	...
	if(filename != NULL && strlen(filename) > 0 && (access(filename,F_OK))!=-1)   {   
		...
		if (mqttConfig.pub_topic_pic_report != NULL && strlen(mqttConfig.pub_topic_pic_report) > 0) {
			...
			sprintf(pub_topic, "WAVE/W7/%s/P/R", btWifiConfig.wifi_mac);
		}
		...
		if (mqttConfig.pub_topic_pic_report_u != NULL && strlen(mqttConfig.pub_topic_pic_report_u) > 0) {
			...
			sprintf(pub_topic_tmp, "WAVE/W7/%s/U/R", btWifiConfig.wifi_mac);
		}
		sprintf(pub_msg, "{\\\"msgId\\\":\\\"%s\\\"\\,\\\"userId\\\":\\\"%s\\\"}", msgId, uuidStr);
		// 第一步：传图
		int ret = pubImage(pub_topic, filename, msgId, btWifiConfig.wifi_mac);
		if (ret == 0) {
			// 第二步：告知图片用户信息
...
int SendMsgToMQTT(char* mqtt_payload, int len) {
	...
	if (mqtt_payload_str != NULL && strncmp(mqtt_payload_str, DEFAULT_HEADER, DEFAULT_HEADER_LEN) == 0) {
		...
		if (msg != NULL && strncmp(msg, "heartbeat", 9) == 0) {
			...
			if (mqttConfig.pub_topic_heartbeat != NULL && strlen(mqttConfig.pub_topic_heartbeat) > 0) {
				...
				sprintf(pub_topic, "WAVE/DEVICE/%s/HEARTBEAT", btWifiConfig.wifi_mac);
			}
		} else if (strncmp(msg, "disconnect", 9) == 0) {
			int ret = disconnectMQTT(MQTT_LINK_ID_DEFAULT);
			return ret;
		} else if (strncmp(msg, "reconnect", 9) == 0) {
			char lwtMsg[50];
			sprintf(lwtMsg, "{\\\"username\\\":\\\"%s\\\"\\,\\\"state\\\":\\\"0\\\"}", btWifiConfig.wifi_mac);
			// 连接MQTT
			int ret = quickConnectMQTT(mqttConfig.client_id, mqttConfig.username, mqttConfig.password, mqttConfig.server_ip, mqttConfig.server_port, mqttConfig.pub_topic_last_will, lwtMsg);
			// // 连接MQTT
			// int ret = connectMQTT(MQTT_LINK_ID_DEFAULT, mqttConfig.server_ip, mqttConfig.server_port, 0);
			sendStatusToMCU(0x02, ret);
			if (ret < 0) {
				log_warn("--------- Failed to reconnect to MQTT\n");
				return -1;
			}
			log_info("--------- reconnect to mqtt done\n");
			// 订阅Topic
			ret = quickSubscribeMQTT(mqttConfig.sub_topic_cmd);
			sendStatusToMCU(0x03, ret);
			if (ret < 0) {
				log_warn("--------- Failed to resubscribe topic\n");
				return -1;
			}
			log_info("--------- resubscribe to mqtt done\n");
			return ret;
		} else {
			...
			if (mqttConfig.pub_topic_cmd_res != NULL && strlen(mqttConfig.pub_topic_cmd_res) > 0) {
				...
				sprintf(pub_topic, "WAVE/DEVICE/%s/INSTRUCTION/RESPONSE", btWifiConfig.wifi_mac);
			}
		...
		if (strncmp(msg, "heartbeat", 9) == 0 && ret == 0) {
			sendStatusToMCU(0x03, ret);
		}
		return 0;
	} else { 
		if ((int)(char)(mqtt_payload[0]) == 0x24) {
			...
			if ((int)(char)(mqtt_payload[1]) == 0x83 && (int)(char)(mqtt_payload[2]) == 0x01) {
				...
				if ((int)(char)(mqtt_payload[3]) == 0x00) {
					...
					// sprintf(pub_msg, "{\\\"msgId\\\":\\\"%s\\\"\\,\\\"mac\\\":\\\"%s\\\"\\,\\\"result\\\":\\\"%d\\\"}", mqtt_msg_id, btWifiConfig.wifi_mac, 0);
					sprintf(pub_msg, "{\\\"msgId\\\":\\\"%s\\\"\\,\\\"mac\\\":\\\"%s\\\"\\,\\\"result\\\":\\\"%d\\\"}", mqtt_msg_id_unlock, btWifiConfig.wifi_mac, 0);
				} else {
					// 远程开锁失败
					printf("---- unlock door fail\n");
					// sprintf(pub_msg, "{\\\"msgId\\\":\\\"%s\\\"\\,\\\"mqtt_payloadult\\\":\\\"%d\\\"\\}", mqtt_msg_id, 1);
					// sprintf(pub_msg, "{\\\"msgId\\\":\\\"%s\\\"\\,\\\"mac\\\":\\\"%s\\\"\\,\\\"result\\\":\\\"%d\\\"}", mqtt_msg_id, btWifiConfig.wifi_mac, 500);
					sprintf(pub_msg, "{\\\"msgId\\\":\\\"%s\\\"\\,\\\"mac\\\":\\\"%s\\\"\\,\\\"result\\\":\\\"%d\\\"}", mqtt_msg_id_unlock, btWifiConfig.wifi_mac, 500);
				}
				// int ret = quickPublishMQTT(mqttConfig.pub_topic, pub_msg);
				int ret = quickPublishMQTT(pub_topic, pub_msg);
				sendStatusToMCU(0x04, ret);
				// mqtt_msg_id[0] = '\0';
				mqtt_msg_id_unlock[0] = '\0';
				return 0;
				...
				if ((int)(char)(mqtt_payload[2]) == 0x0f) {
					...
					if (mqttConfig.pub_topic_record_request != NULL && strlen(mqttConfig.pub_topic_record_request) > 0) {
						...
						sprintf(pub_topic, "WAVE/DEVICE/%s/W7/RECORD/REQUEST", btWifiConfig.wifi_mac);
					}
					int ret = quickPublishMQTT(pub_topic, pub_msg);
					sendStatusToMCU(0x04, ret);
					return ret;
...
#endif
				// int ret = sendImage(filename, uuidStr, 1);
	...
	sprintf(pub_topic, "%s", btWifiConfig.wifi_mac);
	// 命令执行中，有结果上报
	if (strlen(mqtt_msg_id) > 0) {
		...
		sprintf(pub_msg, "{\\\"msgId\\\":\\\"%s\\\"\\,\\\"mac\\\":\\\"%s\\\"\\,\\\"result\\\":\\\"%s\\\"}", mqtt_msg_id, btWifiConfig.wifi_mac, base64_msg);
		...
		if (mqttConfig.pub_topic_cmd_res != NULL && strlen(mqttConfig.pub_topic_cmd_res) > 0) {
			...
			sprintf(pub_topic, "wave/device/%s/instruction/mqtt_payload", btWifiConfig.wifi_mac);
		}
#endif
	} else {
		// 主动上报功能
		if (strncmp(res, DEFAULT_HEADER, DEFAULT_HEADER_LEN) == 0) {
			...
			if (strncmp(msg, "heartbeat", 9) == 0) {
				...
				if (w7_firmware_version != NULL && strlen(w7_firmware_version) > 0) {
					sprintf(pub_msg, "{\\\"msgId\\\":\\\"%s\\\"\\,\\\"mac\\\":\\\"%s\\\"\\,\\\"battery\\\":%d\\,\\\"version\\\":\\\"%s\\\"}", msgId, btWifiConfig.wifi_mac, w7_battery_level, w7_firmware_version);
				} else {
					sprintf(pub_msg, "{\\\"msgId\\\":\\\"%s\\\"\\,\\\"mac\\\":\\\"%s\\\"\\,\\\"battery\\\":%d\\,\\\"version\\\":\\\"%s\\\"}", msgId, btWifiConfig.wifi_mac, w7_battery_level, getFirmwareVersion());
				}
				// sprintf(pub_msg, "msgId:%s\\,mac:%s\\,v:%s", msgId, btWifiConfig.wifi_mac, "1");
...
int analyzeMQTTMsgInternal(char *msg) {
	...
	if (pNext != NULL) {
		...
		if (dataLen == NULL || data == NULL) {
			log_error("analyzeMQTTMsgInternal dataLen is NULL or data is is NULL");
			return false;
		}
		...
		if(fd_act <= 0) {
			...
			if (AT_CMD_SENT == at_state) {
				...
				if (rx_len > 0) {
...
#if 0
								write_config(DEFAULT_CONFIG_FILE, "wifi:wifi_mac", device_mac_from_module);
					...
					if (second != NULL) {
						log_info("################## receive subscribe message from mqtt server %s", second);
						// something get, need to analyze and handle
						// printf("something get %s\n", rx_buf);
						int ret = 0;
						ret = analyzeMQTTMsg(second);
						if (ret == 0 || ret == -2) {
							log_info("#################### receive done, reset rx_buf current at_state is %d", at_state);
							rx_len = 0;
							at_state = AT_CMD_OK;
						}
					}
					second = (char*)strstr(rx_buf, "+MQTTDISCONNECTED:0");
					if (second != NULL && mqtt_init_done == 1) {
						rx_len = 0;
						log_info("################## receive disconnect message from wifi_module");
						char pub_msg[50];
						memset(pub_msg, '\0', 50);
				...
				if (rx_len <= 0) {
					...
					char *second = (char*)strstr(rx_buf, "+MQTTDISCONNECTED:0");
					if (second != NULL && mqtt_init_done == 1) {
						rx_len = 0;
						memset(rx_buf, '\0', UART_RX_BUF_LEN);
						log_info("################## receive disconnect message from wifi_module");
...
int verifyLocalDeviceMac() {
	...
	int device_mac_len = strlen(btWifiConfig.wifi_mac);
	log_info("mac in config is %s\n", btWifiConfig.wifi_mac);
	log_info("default mac is %s\n", DEFAULT_MAC);
	log_info("compare returns %d\n", strncmp(btWifiConfig.wifi_mac, DEFAULT_MAC, 12));
	if (device_mac_len > 0 && strncmp(btWifiConfig.wifi_mac, DEFAULT_MAC, 12) != 0) {
		return update_mac(DEFAULT_CONFIG_FILE, btWifiConfig.wifi_mac);
	}
	return -1;
}
...
void sendImageAtStart() {
	DIR    *dir;
	struct    dirent    *ptr;

	dir = opendir(REG_IMG_DIR);
	log_info("************ sendImageAtStart ************");

	while(dir != NULL && (ptr = readdir(dir)) != NULL) {
		char filename[255];
		memset(filename, '\0', 255);
		char uuidStr[255];
		memset(uuidStr, '\0', 255);
		// int ret = sscanf(ptr->d_name, "ID_%s.jpg", uuidStr);
		int ret = sscanf(ptr->d_name, "ID_%[^.]", uuidStr);
		if (ret > 0 && uuidStr != NULL && strlen(uuidStr) > 0) {
			int len = strlen(ptr->d_name);
			if (strcmp((ptr->d_name + ( len - 4)), ".jpg")==0) {
				sprintf(filename, "%s/%s", REG_IMG_DIR, ptr->d_name);
				log_info("filename: %s\n", filename);
		 		log_info("UUID is %s\n", uuidStr);
				// 不需要通知MCU业务完成，防止MCU下电
				sendImage(filename, uuidStr, 0);
			}
		}
	}
...
// #endif
	...
	// 连接WIFI
	ret = connectWifi(btWifiConfig.ssid, btWifiConfig.password);
	...
	char *msgId = gen_msgId();
	sprintf(timeSync, "{\\\"msgId\\\":\\\"%s\\\"\\,\\\"mac\\\":\\\"%s\\\"}", msgId, btWifiConfig.wifi_mac);
	char pub_topic[100];
	memset(pub_topic, '\0', 100);
	if (mqttConfig.pub_topic_time_sync != NULL && strlen(mqttConfig.pub_topic_time_sync) > 0) {
		...
		sprintf(pub_topic, "WAVE/W7/%s/TIME/REQUEST", btWifiConfig.wifi_mac);
	}
...
void ImageProcCB(void) {
	...
	while (loop) {
		...
		if((access(image_file,F_OK))!=-1)   {   
			...
			int ret = pubImage(pub_topic, msg, gen_msgId(), btWifiConfig.wifi_mac);
#if 0
			if (ret == 0) {
				remove(image_file);
			}
